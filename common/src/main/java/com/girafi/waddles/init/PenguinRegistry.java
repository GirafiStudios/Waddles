...
public class PenguinRegistry {
    ...
    public static final RegistryObject<EntityType<? extends AdeliePenguinEntity>> ADELIE_PENGUIN = registerPenguin("adelie_penguin", () -> AdeliePenguinEntity::new, 0.4F, 0.95F);

    private static <T extends AdeliePenguinEntity> RegistryObject<EntityType<? extends AdeliePenguinEntity>> registerPenguin(String name, Supplier<EntityType.EntityFactory<T>> factory, float width, float height) {
        RegistryObject<EntityType<? extends AdeliePenguinEntity>> penguin = ENTITIES.register(name, () -> EntityType.Builder.of(factory.get(), MobCategory.CREATURE).sized(width, height).clientTrackingRange(64).updateInterval(1).eyeHeight(0.9F).canSpawnFarFromPlayer().build(null));
